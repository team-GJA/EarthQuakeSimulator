...
func JsonHandler(rw http.ResponseWriter, req *http.Request) {
	...
	if vars["mode"] == "manual" {
		...
		randvalue := rand.NormFloat64() - 1
		randvalue2 := rand.Float32()
		scale := float32(randvalue + float64(0.947802 * (5 + 2 * randvalue2)) - 0.004825 * mind)
		switch {
		case scale >= 6.5 :
			output.Scale = "7"
		case scale < 6.5 && scale >= 6.0 :
		...
		for i, _ := range output.Scaleranges {
			output.Scaleranges[i] = int((float32(randvalue) + 0.947802 * (5 + 2 * randvalue2) - (float32(i) + 0.5)) / 0.004825)
			if output.Scaleranges[i] < 0 {
				output.Scaleranges[i] = 0
			}
		}
